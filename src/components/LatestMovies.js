...
const LatestMovies = ({ nowPlaying }) => (
  <div style={{ background: "rgb(255, 255,255, 0.1)", padding: "20px", marginBottom: "20px" }}>
    <Row gutter={16} className='stretch-container'>
      <div className='tag-title' style={{ marginBottom: "30px" }}>
        <h4 style={{ color: "white" }}>
          <b>NOW PLAYING IN THEATERS</b>
        </h4>
      </div>
      {nowPlaying.map(item => (
        <Col xs={24} sm={8} md={6} lg={3} xl={3} key={item.id}>
